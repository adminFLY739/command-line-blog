            ...
            if (Objects.equals(index, "#")) {
                ...
                try {
                    ...
                    if (indexToRemove >= 0 && indexToRemove < filteredArray.length) {
                        ...
                        List<Article> updatedArticles = Arrays.stream(articles).filter(article -> !article.equals(articleToRemove)).collect(Collectors.toList());
                        Writer writer = new FileWriter(filePath);
                        gson.toJson(updatedArticles, writer);
